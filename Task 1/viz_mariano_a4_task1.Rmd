...
stream_chem_mean <- stream_chem_sb %>% 
  group_by(site_code) %>% #gropu by site
  summarise(mean_nh4 = mean(nh4_uM, na.rm = TRUE),
            ...
            mean_spec_cond = mean(spec_cond_uSpercm, na.rm = TRUE) #set new col with mean values
    ) %>% 
  na.omit() 
...
```{r}
#Find the Euclidean distance with `stats::dist()`:
